...
- Process management System Calls
  - Fork - forks a **_parent_** process to a **_child_** process (copies parent -> child)

  - Exec - rewrites the **_parent_** process with its own memory image and instructions

  - Exit - Child processes stops executing and becomes **_zombies_**

  - Wait - **_Reaps(cleans the memory)_** of the child processes(zombies)

  - > Sometimes the parents exit before the child and they become orphan processses which is when foster parent(**_generally init_**) will adapt them and reap them, if not reaped, exhausts system memory

> OS Scheduler loops over **_ready_** processes.
