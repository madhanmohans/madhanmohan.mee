...
3. Data validation strategy 1. we won’t have enough data to test 2. to test mill, 2 mill records 3. how to maintain data quality to new system 4. whenever data bricks object are deployed in prod, we compare gold layer objects with prev systems and we map each row 5. Row count, row-to-row, last 3 months comp 6. will need compute but will be beneficial 7. when data pipelines built → sit runs → generates report
   commercial ops   
